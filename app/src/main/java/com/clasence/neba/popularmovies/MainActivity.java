...
public class MainActivity extends AppCompatActivity implements MovieAdapter.CustomClickListener{
    //progress bar
        ...
        if(savedInstanceState!=null && savedInstanceState.containsKey(ALL_MOVIES)){
            ...
            if (CheckInternetConnection.isNetworkAvailable(getApplicationContext())) {
                ...
                try {
                    ...
                    getMoviesAsync = new LoadMovies().execute(url);
    ...
    protected void onDestroy() {
        super.onDestroy();
        if(getMoviesAsync!=null){
            //cancel asynctask on activity destroy to avoid crashing
            getMoviesAsync.cancel(true);
        }
    }
    ...
    /**
     * Class to load data from themoviedb api
     */

    private class LoadMovies extends AsyncTask<URL,Void,String>{
        private HttpURLConnection urlConnection;

        //holds error message if any network error occurs
                ...
                try {
                    ...
                    if (results.length() > 0) {
                        ...
                        for (int i = 0; i < results.length(); i++) {
                            ...
                            MovieModel movieModel = new MovieModel(movieId, movieTitle, voteAverage, overview, releaseDate, posterPath, popularity, video, voteCount);
